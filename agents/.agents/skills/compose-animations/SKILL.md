...
1. Choose the smallest API from the table below.
1. Check lifecycle semantics: should hidden content leave composition, keep focus/state, or only become transparent?
1. Check identity: for state-holder wrappers, choose `AnimatedContent.contentKey` by visual shape rather than payload churn.
1. Check performance: keep frame-rate animation values as `State` and read them in layout/draw block modifiers when possible.
1. Escalate to `Animatable` or lower-level APIs only when target-state animation cannot express the motion.
