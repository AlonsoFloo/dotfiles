...
1. For each module, read its `build.gradle.kts` to identify which plugins are applied
1. Check if the project uses a Gradle version catalog (`gradle/libs.versions.toml`). If it exists,
...
1. Read `gradle/wrapper/gradle-wrapper.properties` for the Gradle version
1. Check `gradle.properties` for any existing workarounds (`android.enableLegacyVariantApi`)
1. Check for `org.jetbrains.kotlin.android` plugin usage — AGP 9.0 has built-in Kotlin and this plugin must be removed
1. Check for `org.jetbrains.kotlin.kapt` plugin usage — incompatible with built-in Kotlin, must migrate to KSP or `com.android.legacy-kapt`
1. Check for third-party plugins that may be incompatible with AGP 9.0 (see "Plugin Compatibility" section below)
...
1. **Remove `org.jetbrains.kotlin.android`** plugin if present (AGP 9.0 has built-in Kotlin support)
1. **Migrate DSL**: Move config from top-level `android {}` block into `kotlin { android {} }`:
   ```kotlin
   kotlin {
       android {
   ...
   ```
1. **Rename source directories** (only if the module uses classic Android layout instead of KMP layout):
...
1. **Move dependencies** from top-level `dependencies {}` into `sourceSets`:
   ```kotlin
   kotlin {
       sourceSets {
   ...
   ```
1. **Enable resources** explicitly if the module uses Android or Compose Multiplatform resources:
   ```kotlin
   kotlin {
       android {
   ...
   ```
1. **Enable Java** compilation if module has `.java` source files:
   ```kotlin
   kotlin {
       android {
           withJava()
   ...
   ```
1. **Enable tests** explicitly if the module has unit or instrumented tests:
   ```kotlin
   kotlin {
       android {
   ...
   ```
1. **Update Compose tooling dependency**:
   ```kotlin
   // Old:
   ...
   ```
1. **Publish consumer ProGuard rules** explicitly if applicable:
   ```kotlin
   kotlin {
       android {
   ...
   ```
1. **Resolve Sub-dependency Variants (Product Flavors / Build Types)**:
...
1. **Create `androidApp` module** with its own `build.gradle.kts`:
   ...
   ```
1. **Move Android entry point code** from `src/androidMain/` to `androidApp/src/main/`:
...
1. **Add to `settings.gradle.kts`**: `include(":androidApp")`
1. **Add to root `build.gradle.kts`**: plugin declarations with `apply false`
1. **Convert original module** from application to library using Path A steps
1. **Ensure different namespaces**: app module and library module must have distinct namespaces
1. **Remove from shared module**: `applicationId`, `targetSdk`, `versionCode`, `versionName`
1. **Update IDE run configurations**: change the module from the old module to `androidApp`
...
1. **Extract `desktopApp`** (if desktop target exists):
...
1. **Extract `webApp`** (if wasmJs/js target exists):
...
1. **AGP version** — update to 9.0.0+ and add the KMP library plugin.
...
1. **JDK** — ensure JDK 17+ is used (required by AGP 9.0)

1. **SDK Build Tools** — update to 36.0.0:

   ```
   Install via SDK Manager or configure in android { buildToolsVersion = "36.0.0" }
   ```

1. **Review gradle.properties** — remove error-causing properties and review changed defaults (see "Gradle Properties Default Changes" section)
...
1. Enable per migrated module by applying the opt-in plugin:
   ```kotlin
   plugins {
       id("com.android.built-in-kotlin") version "AGP_VERSION"
   }
   ```
1. Follow Steps 1-4 for that module
1. Once all modules are migrated, remove `android.builtInKotlin=false` and all `com.android.built-in-kotlin` plugins
...
1. All platform targets build successfully (Android, iOS via `xcodebuild`, Desktop, JS/Wasm)
1. `./gradlew :shared:allTests` and Android unit tests pass
1. No `com.android.library` or `com.android.application` in KMP modules
1. No `org.jetbrains.kotlin.android` in AGP 9.0 modules
1. Source sets use correct names (`androidMain`, `androidHostTest`, `androidDeviceTest`)
1. No deprecation warnings about variant API or DSL
