...
1. Change Module to `androidApp`
1. Ensure the launch activity is `com.example.app.MainActivity`
...
1. **Update the Run Script build phase** in `project.pbxproj` (or via Xcode > Build Phases > Run Script) to reference the new module:
...
1. **Update Swift imports** — in all `.swift` files, change the framework import to match `baseName`:
...
1. **Update the app struct name** if it was tied to the old module name. The `@main` struct name in your SwiftUI app entry point is independent of the framework name, but if it referenced the old name, rename it:
...
1. **Update framework search paths** in Xcode Build Settings if they reference the old module directory path.
