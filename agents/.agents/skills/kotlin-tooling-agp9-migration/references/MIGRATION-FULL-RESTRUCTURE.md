...
1. Put all shared code in `commonMain` and rely on the JVM dependency from `desktopApp`.
...
1. **Gradle task path:** Update the Run Script build phase in `project.pbxproj` (or via Xcode > Build Phases):
...
1. **App struct name:** If the SwiftUI `@main` struct was named after the old module (e.g., `ComposeAppApp`), rename it to something appropriate for your project.

1. **Framework search paths:** Update Build Settings if they reference the old module directory path.

1. **Cocoapods (if used):** Update the pod spec name:
