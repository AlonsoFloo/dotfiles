...
1. **Check each against the tables above**
1. **If any plugin is broken without workaround**, inform the user — they may need to wait for a plugin update or remove it
1. **If plugins need opt-out flags**, add them to `gradle.properties` and note them as temporary workarounds
1. **Update plugin versions** to their AGP 9.0-compatible versions before or during migration
