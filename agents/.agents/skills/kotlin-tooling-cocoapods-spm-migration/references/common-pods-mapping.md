...
1. **Must use `isStatic = true`.** With a dynamic framework, the K/N linker creates `@rpath/FirebaseCore.framework/FirebaseCore` load instructions. Firebase SPM products are static libraries — their `.framework` bundles are not embedded in the app bundle. At runtime, `dyld` crashes with `Library not loaded`. Switching to `isStatic = true` embeds all symbols and defers unresolved framework flags to the final Xcode link.

1. **iOS test tasks may fail.** The K/N test runner cannot find Firebase frameworks outside of Xcode context. You may need to disable iOS test tasks:
...
1. **Check CocoaPods spec** - The `source` field often points to the Git URL
1. **Search Swift Package Index** - https://swiftpackageindex.com/
1. **Check library documentation** - Many libraries document SPM installation
...
1. Run `./gradlew build`
1. Check build errors for available class names
1. Or check the library's `module.modulemap` file in its source
...
1. CocoaPods `version = "X.Y.Z"` (without `~>`) is an exact pin — use `exact("X.Y.Z")` in SPM, not `from()`. Only use `from()` when the CocoaPods spec used optimistic versioning (`~>`).
1. Check the GitHub releases page to confirm the exact version is available as an SPM release
