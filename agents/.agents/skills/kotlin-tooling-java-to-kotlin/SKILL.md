...
1. **Nullability & mutability audit** — val/var, nullable types
1. **Collection type conversion** — Java mutable → Kotlin types
1. **Idiomatic transformations** — properties, string templates, lambdas
...
1. **Sort by dependency order** — convert leaf dependencies first (files that don't
   import other files in the conversion set), then work up to files that depend on them
1. **Convert one file at a time** — apply the full workflow (steps 0-4) for each
1. **Track progress** — report which files are done, which remain
1. **Handle cross-references** — after converting a file, update imports in other Java
