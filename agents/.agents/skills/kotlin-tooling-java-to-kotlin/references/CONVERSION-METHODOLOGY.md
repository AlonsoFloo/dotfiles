...
1. Convert `nickname` getter/setter → Kotlin property with `@field:Nullable` and
   `@get:Nullable`.
1. Drop `Objects.requireNonNull` — Kotlin's type system enforces non-null.
1. Preserve all imports even if now unused (invariant 3).
