...
1. **@JsonCreator**: Java's `@JsonCreator` static factory or constructor → Kotlin primary
...
1. **@JsonIgnore**: Preserve exactly. Use `@get:JsonIgnore` or `@field:JsonIgnore`
   depending on original target.

1. **@JsonDeserialize / @JsonSerialize**: Preserve exactly with correct site targets.

1. **@JsonInclude**: Preserve on class or property level.

1. **@JsonFormat**: Preserve with `@field:JsonFormat` site target.

1. **Jackson Kotlin Module**: Note that projects using Jackson with Kotlin should add
...
1. **Builder pattern with @JsonPOJOBuilder**: Replace with primary constructor +
