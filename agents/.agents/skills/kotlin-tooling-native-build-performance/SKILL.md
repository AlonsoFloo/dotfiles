...
1. Measure before and after with the same command and the same build state.
...
1. Find the command the user actually waits for: a script, a CI step, or the
   Gradle invocation inside an Xcode build phase. Optimize that command, not
   a task you picked yourself.

1. Run it twice when practical. The first build downloads Kotlin/Native
...
1. If you cannot run the build (no macOS host, no Xcode), analyze logs, build
...
1. **Build only what the feedback loop needs** — one specific task per loop,
...
1. **Cut export and generated-code cost** — drop `transitiveExport`, narrow
...
1. **Experimental switches last, with the user's agreement**:
