...
1. Re-evaluate which isolation boundaries the type crosses.
1. Route to the matching reference file for a deeper fix.
1. If the fix may change behavior, document the invariant and add verification steps.
...
1. **Fix** — Address one category of error at a time (e.g., all Sendable issues first)
1. **Rebuild** — Confirm the fix compiles cleanly before moving on
1. **Test** — Run the test suite to catch regressions (`swift test` or Cmd+U)
1. **Only proceed** to the next file/module when all diagnostics are resolved
...
1. Build and clear one category of errors before moving on. Do not batch unrelated fixes into the same change.
1. Run tests, especially actor-, lifetime-, and cancellation-sensitive tests.
1. Use Instruments for performance claims instead of guessing.
1. Verify deallocation and cancellation behavior for long-lived tasks.
1. Check `Task.isCancelled` in long-running operations.
1. Never use semaphores or ad hoc locking in async contexts when actor isolation or `Mutex` would express ownership more safely.
