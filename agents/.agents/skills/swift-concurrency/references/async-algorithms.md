...
1. **Combine streams** with merge/combineLatest instead of manual state management
1. **Use AsyncChannel** for multi-consumer scenarios with backpressure
1. **Ensure Sendable conformance** when using operators across isolation boundaries
1. **Leverage cancellation** - Task cancellation propagates through all operators
1. **Choose right tool**: AsyncAlgorithms for complex streams, AsyncStream for bridging callbacks
1. **Avoid manual sleep loops** - use AsyncTimerSequence instead
