...
1. **Update method signature** - add `async`, remove completion parameter
1. **Replace closure calls with await** - use URLSession async APIs
1. **Remove optional unwrapping** - async APIs return non-optional values
1. **Simplify error handling** - use do-catch instead of nested closures
1. **Return directly** - compiler enforces return values
