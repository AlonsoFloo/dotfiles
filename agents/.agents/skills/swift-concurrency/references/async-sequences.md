...
1. **Use buffer policies wisely** - Match your use case (latest value vs all values)
1. **Handle cancellation** - Set `onTermination` for cleanup
1. **Single consumer** - Don't share streams across multiple consumers
1. **Prefer streams over closures** - More composable and cancellable
1. **Check Task.isCancelled** - Respect cancellation in custom sequences
1. **Use throwing variant** - When operations can fail
1. **Consider regular async** - If only returning single value
