...
1. **Use perform { }** - don't access context directly
1. **@MainActor for view context** - enforce main thread
1. **@concurrent for background** - force background execution
1. **Manual code generation** - control isolation
1. **Keep it simple** - avoid custom executors unless needed
1. **Enable Core Data debugging** - catch thread violations
1. **Merge changes automatically** - `automaticallyMergesChangesFromParent = true`
1. **Use background contexts** - for heavy operations
1. **Test with Thread Sanitizer** - catch violations early
...
1. **Mark entities as nonisolated** if using default @MainActor
1. **Wrap Core Data access** in CoreDataStore
1. **Use @MainActor** for view context operations
1. **Use @concurrent** for background operations
1. **Pass NSManagedObjectID** between contexts
1. **Test with debugging enabled**

### For new projects

1. **Start with simple pattern** (CoreDataStore)
1. **Manual code generation** from the start
1. **Consider DAOs** if heavy cross-context usage
1. **Enable strict concurrency** early
