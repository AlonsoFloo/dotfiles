...
1. **Override requirement** — base class API is `async`.
1. **`@concurrent` requirement** — stays `async` even without `await`.
1. **Accidental/legacy `async`** — no caller needs async semantics.

### Preferred fixes (order)

1. **Remove `async`** (and adjust call sites) when no async semantics are needed.
1. If `async` is required (protocol/override/@concurrent):
...
1. Use `@MainActor` isolation if it's UI-related
1. Capture only Sendable values instead of `self`
1. Use `@unchecked Sendable` with documented safety invariant (last resort)
...
1. Return Sendable projections (IDs, copies of data)
1. Keep processing within the actor's isolation
...
1. Use `await MainActor.run { }` for one-off access
1. Reconsider if the property truly needs @MainActor isolation
...
1. **What** invariant makes it safe
1. **When** it can be removed (link to migration ticket)
