...
1. Task starts
1. `viewModel = nil` (but object not deallocated)
1. Task completes
1. ViewModel finally deallocated
...
1. Look for cycles in object graph
...
1. **Use guard let self** in async sequences
1. **Cancel tasks explicitly** when possible
1. **Add deinit logging** during development
1. **Test object deallocation** in unit tests
1. **Use Memory Graph** to verify no cycles
1. **Document lifetime expectations** in comments
1. **Prefer cancellation** over weak self when possible
1. **Avoid nested strong captures** in task closures
1. **Use isolated deinit** for cleanup (Swift 6.2+)
