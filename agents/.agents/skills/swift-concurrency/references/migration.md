...
1. See 50+ errors and warnings
1. Fix a bunch of them
1. Rebuild and see 80+ new errors appear
...
1. Set to **Migrate** (temporary setting)
1. Build the project
1. Warnings appear with **Apply** buttons
1. Click Apply for each warning
...
1. **Add Async Alternative**: Rewrites method as async, keeps original
1. **Convert Function to Async**: Replaces method entirely
...
1. **Check for updates first**: Library might have a newer version with concurrency support
1. **Document why**: Add a comment explaining why it's needed
1. **Revisit regularly**: Set reminders to check if library has been updated
...
1. **Be incremental**: Use the three strict concurrency levels (Minimal → Targeted → Complete)
1. **Use tooling**: Leverage Xcode refactoring and `swift package migrate`
1. **Create checkpoints**: Small, focused PRs that can be merged quickly
1. **Stay positive**: The concurrency rabbit hole is real, but manageable with the right habits
1. **Think differently**: Let go of the threading mindset; trust the compiler
