...
1. Immutable stored properties only
1. All properties Sendable
1. No superclass or `NSObject` only
...
1. If widely used → use `@unchecked Sendable`, file migration ticket
...
1. **Use actors for mutable state** - automatic thread-safety
1. **Avoid @unchecked Sendable** - use only for proven thread-safe code
1. **Mark public types explicitly** - don't rely on implicit conformance
1. **Ensure all members Sendable** - one non-Sendable breaks the chain
1. **Use @MainActor for UI types** - simple isolation for view models
1. **Capture immutably** - use capture lists for mutable variables
1. **Test with Thread Sanitizer** - catches runtime data races
1. **File migration tickets** - track @unchecked Sendable usage
