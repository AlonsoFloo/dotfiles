...
1. New task starts with updated value
...
1. **Use structured concurrency** (avoid detached tasks)
1. **Leverage SwiftUI's `.task` modifier** for view-bound work
1. **Choose the right tool**: `async let` for fixed, TaskGroup for dynamic
1. **Handle errors explicitly** in throwing task groups
1. **Set priority only when needed** (inherit by default)
1. **Don't mutate task groups** from outside their creation context
