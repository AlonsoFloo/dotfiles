...
1. **Mark tests with correct isolation** - @MainActor when needed
1. **Use confirmations over continuations** - when structured concurrency allows
1. **Serialize tests with main serial executor** - avoid flaky tests
1. **Test cancellation explicitly** - ensure proper cleanup
1. **Verify deallocation** - catch retain cycles early
1. **Use Task.yield() strategically** - control execution in tests
1. **Avoid sleep in tests** - use continuations/confirmations instead
1. **Test actor isolation** - verify thread safety
1. **Keep tests deterministic** - avoid timing dependencies
