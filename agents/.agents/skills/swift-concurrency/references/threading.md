...
1. **Task scheduling**: Tasks scheduled onto available threads
1. **Suspension**: At `await`, task suspends, thread freed for other work
1. **Resumption**: Task resumes on any available thread (not necessarily the same one)
...
1. **State can change** - mutable state may be modified during suspension
1. **Actor reentrancy** - other tasks can access actor during suspension
...
1. Debug → Pause
1. Check Debug Navigator for thread info
...
1. **Trust the system** - Swift optimizes thread usage
1. **Use @MainActor for UI** - clear, explicit main thread execution
1. **Minimize suspension points in actors** - avoid reentrancy bugs
1. **Complete state changes before suspending** - prevent inconsistent state
1. **Use priorities as hints** - not guarantees
1. **Make types Sendable** - safe across thread boundaries
1. **Enable Swift 6.2 features** - easier migration, better defaults
1. **Set default isolation for apps** - reduce false warnings
1. **Don't force thread switching** - let Swift optimize
...
1. Enable `NonisolatedNonsendingByDefault`
1. Use `@concurrent` for explicit background work

### For existing projects

1. Gradually enable Swift 6 language mode
1. Consider default isolation change
1. Use `@concurrent` to maintain old behavior where needed
